...
try:
	for team in list_teams:
		for week in [a for a in range(14)]:
			response = requests.get("https://api.collegefootballdata.com/plays", params={"seasonType": "regular", "year": 2018, "offense": team, "week": week})
			print(team + " week " + str(week) + " " + str(response.status_code) + response.url)
			if response.status_code == 200:
				for play in response.json():
					opp_team = play["defense"]
					if play["play_type"] in play_types:
						if play["distance"] != 0:
							if play["play_type"] == "Rush":
								success[team][week]["Rushes"] += 1
								if opp_team in list_teams:
									defense_success[opp_team][week]["Rushes"] += 1
								if play["down"] == 1:
									success[team][week]["1st Downs"] += 1
									if opp_team in list_teams:
										defense_success[opp_team][week]["1st Downs"] += 1
									if (play["yards_gained"] / play["distance"]) > .5:
										success[team][week]["1st Successes"] += 1
										success[team][week]["Rush Successes"] += 1
									else:
										if opp_team in list_teams:
											defense_success[opp_team][week]["1st Successes"] += 1
											defense_success[opp_team][week]["Rush Successes"] += 1
								if play["down"] == 2:
									success[team][week]["2nd Downs"] += 1
									if opp_team in list_teams:
										defense_success[opp_team][week]["2nd Downs"] += 1
									if (play["yards_gained"] / play["distance"]) > .7:
										success[team][week]["2nd Successes"] += 1
										success[team][week]["Rush Successes"] += 1
									else:
										if opp_team in list_teams:
											defense_success[opp_team][week]["2nd Successes"] += 1
											defense_success[opp_team][week]["Rush Successes"] += 1
								if play["down"] == 3:
									success[team][week]["3rd Downs"] += 1
									if opp_team in list_teams:
										defense_success[opp_team][week]["3rd Downs"] += 1
									if (play["yards_gained"] / play["distance"]) > 1.0:
										success[team][week]["3rd Successes"] += 1
										success[team][week]["Rush Successes"] += 1
									else:
										if opp_team in list_teams:
											success[opp_team][week]["3rd Successes"] += 1
											success[opp_team][week]["Rush Successes"] += 1
								...
								if opp_team in list_teams:
									defense_success[opp_team][week]["Passes"] += 1
								...
								if play["down"] == 1:
									...
									if (play["yards_gained"] / play["distance"]) > .5:
										...
										if opp_team in list_teams:
											defense_success[opp_team][week]["1st Successes"] += 1
											defense_success[opp_team][week]["Pass Successes"] += 1
								if play["down"] == 2:
									success[team][week]["2nd Downs"] += 1
									if opp_team in list_teams:
										defense_success[opp_team][week]["2nd Downs"] += 1
								...
								if play["down"] == 3:
									...
									if (play["yards_gained"] / play["distance"]) > 1.0:
										...
										if opp_team in list_teams:
											defense_success[opp_team][week]["3rd Successes"] += 1
											defense_success[opp_team][week]["Rush Successes"] += 1

	success.to_excel("C:\\users\\jva12\\Documents\\CFB Poll\\Offense.xlsx")
	defense_success.to_excel("C:\\users\\jva12\\Documents\\CFB Poll\\Defense.xlsx")
